...
强制执行同一校验，默认使用 `profile=default`、`data_manifest.yaml` 和
`data_bundle.lock.json`；失败即终止。使用 CNE6L 或其他数据包时，可在 YAML 顶层显式写：
...
| `run.<run_id>.manifest.json` | 单次运行不可覆盖清单：绑定提交/脏差异、有效配置、数据锁、真实 Alpha、运行环境和全部产物 SHA-256 |
...
`optimization` 保存候选/成功/失败期数及 `post_solve_validation`（$10^{-5}$ 绝对容差、
逐期最大残差和失败明细），`benchmark_quality` 保存基准来源、逐期快照日、漂移方式与
...
合计）列出累计贡献、年化贡献、占主动收益比例、t 统计、年化波动。累计贡献使用
`Π(1+组合收益)/Π(1+基准收益)-1` 的精确几何超额口径；逐日 `active_return` 仍保留
算术差用于 Barra 残差自检，并另存 `relative_active_return`。
