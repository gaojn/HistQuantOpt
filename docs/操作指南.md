...
只跑 `hqopt run`（不重新拉数）时只依赖 B1~B4 这四类本地文件；重新拉数/刷新缓存才需要连 ClickHouse（见 1.4）。

> ⚠️ **已知风险（2026-07-16）**：`hqopt data sync` 底层查询的 ClickHouse 视图
> `vw_ashare_daily_backtest` **当前在服务器上不存在**（全库 `system.tables` 搜索 0 命中，
> 疑似被下线/改名，未及通知）。**本地 `data/cache/` 已有数据覆盖到 2026-06-11**，
> 在此之前不影响日常 `hqopt run`；但**在有人确认视图现状/补建之前，不要执行
> `hqopt data sync` 去刷新缓存**——会直接报错。需要更新缓存时，先找数据侧确认该
> 视图的替代方案，或联系维护者（本仓库审查记录见根目录审查报告）。
...
| 数据 | 位置 | 来源 / 生成 |
|---|---|---|
| A股全市场行情 | `data/cache/ashare_daily_<year>.parquet` | `hqopt data sync`（the_quant；⚠️ 见 1.2 已知风险） |
