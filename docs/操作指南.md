...
| CNE6 风险模型 | `data/barra_cne6/`、`data/barra_cne6_L/` | `hqopt data cne6`（test_barra_cne6_gao） |
...
python scripts/verify_data_bundle.py --profile attribution_long # CNE6L + 收益归因
```
...
`actual_weights.parquet` 和执行统计 JSON。
...
> 直接取消旧目标，不再尝试旧单，只按真实股数估值；收盘提交新目标并重置状态/计数，
> 下一交易日才是新目标的 T+1。若快照、风险覆盖或求解失败导致没有新权重行，则该日
> 不构成有效调仓，旧目标保持激活并正常尝试。
>
...
| `batch_execution_stats.json` | 常规权重的成交、优化期数与 `alpha_quality` 审计统计；自定义权重名改用 `<weights stem>.batch_execution_stats.json` |
| `nav.parquet` / `turnover.parquet` | 逐日净值与实际成交换手 |
| `actual_weights.parquet` | 成交、费用和价格漂移后的逐日实际股票权重 |
| `execution_stats.json` | 回测执行统计：过期订单、未成交金额、最终股数和订单状态 |
| `attribution_execution_stats.json` | 归因成交重放统计；独立命名，避免覆盖回测统计 |
...
`optimization` 保存候选/成功/失败期数，`benchmark_quality` 保存基准来源、逐期快照日、漂移方式与回退原因，`alpha_quality` 保存标准化、陈旧阈值、最大
陈旧度、跳过/零方差期数及逐期 as-of 日期，并与权重一起受 manifest 哈希绑定。
...
| 信息比率 IR | `年化超额 / TE` |
| Calmar | 对应区间累计收益 / 同期最大回撤（不年化） |
| 超额 Calmar | 全期累计几何超额 / 超额最大回撤（不年化） |
...
| `--start` / `--end` | 必填 | 归因区间 |
| `--index` | `zz1000` | `hs300/zz500/zz1000` 用成分权重；其余（如 `all`/`csiall`）退回全市场等权基准 |
| `--benchmark-weight-source` | `official_drift` | 可选 `official_frozen`（历史冻结口径）或 `reconstruct`；应与生成权重时一致 |
| `--out-dir` | 不落地 | 输出归因汇总/逐日/逐因子明细，并保存 `attribution_execution_stats.json` |
...
**输出的归因表**：按归因项（默认S为20个风格，L为16个风格 + 行业合计 + Country + 特质(选股) + 残差 +
