...
- **失败处理**：快照、风险覆盖或求解失败导致没有新权重行时，不提交新目标；该日不
  构成有效调仓，旧目标保持激活并正常尝试
- **求解后门禁**：`optimal` / `optimal_inaccurate` 只代表求解器状态，不直接授权发布。
  权重清理后重新计算预算、纯多头、单票上限、冻结/强零/sell_only、换手、行业、风格、
  成分股下限，以及指增主动权重/L2 等全部硬约束；任一绝对超限大于 $10^{-5}$ 即按
  infeasible 拒绝该期。逐期最大残差和失败明细写入 `batch_execution_stats.json`。
...
- T+3 后残余订单过期，已成交股数和部分成交股数均保留；累计过期笔数与未成交金额
  写入执行统计 JSON。
- 候选调仓日盘中先按当日行情执行旧目标并估值，再用执行后的实际持仓生成新目标。
  只有新目标成功并在收盘提交时，才覆盖旧目标的残余 pending 状态、重置股票状态；
  下一交易日才作为新目标的 T+1 开始三次尝试计数。
...
- **Calmar 采用行业标准口径**：年度行 = 当年年化收益(CAGR) / 当年最大回撤；全期行 =
  全期年化收益 / 全期最大回撤；超额 Calmar 同理使用年化几何超额。分子与报告
  “收益”列的年化值（全期行标 `*`）一致，跨不同长度区间可比。不足一年的分组
  年化后数值会明显放大，属预期现象，解读时结合样本天数判断。
