"""
量化选股回测 Demo（alpha_max，全市场选股，CNE6 风险模型，真实执行）。

完整闭环：加载行情 → 全市场范围内最大化 alpha 优化（行业上限/分散度/风格暴露约束）
→ 真实回测（T+1 VWAP/涨跌停/成本）→ 生成 HTML 报告 + 落地数据 parquet。
基准用官方万得全A 指数收盘价（data/指数收盘价信息.csv）。
...
CONFIG      = "configs/alpha_max_demo.yaml"
BENCH       = "winda"        # 对标万得全A（官方收盘价列）
BENCH_NAME  = "万得全A"
ALPHA_PATH  = Path("alphas/alpha_vwap5_ic0.12_icir1.2_decay0.80.parquet")
OUT_DIR     = Path("output/alpha_max_demo")
...
def main() -> None:
    ...
    print(f"\n{'='*70}")
    print(f"  量化选股回测（alpha_max，全市场）  |  {ALPHA_PATH.stem}")
    ...
    print(f"\n[3] 加载 {BENCH_NAME} 官方指数收盘价作为基准...")
    bm_ret = (
        load_index_returns(BENCH, start_date, end_date)
    ...
    report_path = generate_html_report(
        ...
        title=f"量化选股 alpha_max（全市场，对标{BENCH_NAME}，{ALPHA_PATH.stem}，真实执行）",
    )
