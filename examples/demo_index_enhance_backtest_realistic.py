...
输入  : output/zz500_enhance_weights.parquet
输出  : output/zz500_enhance_report_realistic.html

运行：
    python examples/demo_index_enhance_backtest_realistic.py
"""
...
def main() -> None:
    print(f"\n{'='*60}")
    print(f"  {INDEX_NAME} 指数增强回测（真实执行模式）")
    ...
    nav_out = out_dir / f"{INDEX}_enhance_nav_realistic.parquet"
    pd.DataFrame({
    ...
    report_path = generate_html_report(
        result,
        output_path=out_dir / f"{INDEX}_enhance_report_realistic.html",
        title=f"{INDEX_NAME} 指数增强组合回测报告（真实执行）",
    )
    print(f"\n  净值已保存     : {nav_out}")
