...
输入  : output/batch_weights_alpha_max.parquet
输出  : output/hs300_alpha_max_nav_realistic.parquet
        output/hs300_alpha_max_report_realistic.html

运行：
    python examples/demo_backtest_realistic.py
"""
...
WEIGHT_PATH = Path(f"output/{INDEX}_alpha_max_weights.parquet")


def main() -> None:
    print(f"\n{'='*60}")
    print(f"  {INDEX_NAME} 量化多头回测（真实执行模式）")
    ...
    nav_out = out_dir / f"{INDEX}_alpha_max_nav_realistic.parquet"
    pd.DataFrame({
    ...
    report_path = generate_html_report(
        result,
        output_path=out_dir / f"{INDEX}_alpha_max_report_realistic.html",
        title=f"{INDEX_NAME} 量化多头组合回测报告（真实执行）",
    )
    print(f"\n  净值已保存     : {nav_out}")
