...
class MarketSnapshot:
    ...
    sell_only: pd.Series | None = None       # index=ticker, value=bool，True=只卖不买（掉池持仓票）
    ...
    @property
    def sell_only_mask(self) -> np.ndarray:
        """只卖不买掩码，shape=(N,)。字段为 None 时返回全 False。"""
        if self.sell_only is None:
            return np.zeros(self.n_stocks, dtype=bool)
        return self.sell_only.reindex(self.tickers).fillna(False).values.astype(bool)
